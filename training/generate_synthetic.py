...
Assistant ต้องตอบแบบมีเอกลักษณ์ตามบุคลิก ไม่ใช่ AI ทั่วไป"""
    ...
    try:
        ...
        if start == -1 or end == 0:
            reason = content[:200].replace("\n", "\\n")
            print(f"    (no JSON array) {reason}")
            return None
...
def _preview(content: str, max_len: int = 80) -> str:
    """First line or truncated snippet of an assistant reply, safe for any console."""
    for line in content.splitlines():
        stripped = line.strip()
        if stripped:
            preview = stripped[:max_len] + ("..." if len(stripped) > max_len else "")
            try:
                preview.encode(sys.stdout.encoding or "utf-8")
