"""Scrape Pantip threads with nested comments as conversation pairs."""
import json
import time
...
def get_topic_urls(tag: str, pages: int = 10) -> list[str]:
    """Collect topic URLs from a tag's listing pages."""
    urls = []
    for page in range(1, pages + 1):
        url = f"{BASE_URL}/tag/{tag}?page={page}"
        resp = requests.get(url, headers={"User-Agent": "NeuroThai/1.0"})
...
def parse_topic(topic_url: str) -> list[dict] | None:
    """Extract topic body and nested comments, return conversation pairs."""
    resp = requests.get(topic_url, headers={"User-Agent": "NeuroThai/1.0"})
    if resp.status_code != 200:
        return None
    ...
    for comment in comments:
        ...
        if len(reply_text) < 20 or len(post_text) < 20:
            continue
        pairs.append({
            "messages": [
        ...
        # Also extract nested replies as multi-turn
        nested = comment.select(".comment-item")
        for nest in nested:
            nest_text = nest.select_one(".display-post-story")
            if nest_text:
                nested_reply = nest_text.get_text(strip=True)
                if len(nested_reply) >= 20:
                    pairs[-1]["messages"].append(
                        {"role": "assistant", "content": nested_reply[:2048]}
                    )
